const MATERIALS=["PLA","PETG","ASA","ABS","TPU","PA","PC","PVA","Autre"];
const BRANDS=["Bambu Lab","eSUN","SUNLU","Polymaker","PolyTerra","Overture","Elegoo","Creality","Anycubic","Prusament","ColorFabb","FormFutura","Fiberlogy","Fillamentum","Extrudr","Spectrum","AzureFilm","Devil Design","Geeetech","JAYO","ERYONE","AMOLEN","Hatchbox","MatterHackers","Proto-pasta","Recreus","Kimya","Nanovia","Arianeplast","Francofil","Sakata 3D","Rosa3D","3DJake","QIDI Tech","Raise3D","Ultimaker","Autre"];
const DB="filament-stock-v2", VER=1; let db;
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
function openDB(){return new Promise((res,rej)=>{let r=indexedDB.open(DB,VER);r.onupgradeneeded=()=>{let d=r.result;if(!d.objectStoreNames.contains("spools"))d.createObjectStore("spools",{keyPath:"id"});if(!d.objectStoreNames.contains("movements"))d.createObjectStore("movements",{keyPath:"id"});};r.onsuccess=()=>{db=r.result;res()};r.onerror=()=>rej(r.error)})}
...
function put(store,obj){return new Promise((res,rej)=>{let r=db.transaction(store,"readwrite").objectStore(store).put(obj);r.onsuccess=()=>res();r.onerror=()=>rej(r.error)})}
...
function filtered(sp,prefix=""){
 ...
 return sp.filter(s=>{let hay=[s.brand,s.material,s.variant,s.colorName].join(" ").toLowerCase(),p=pct(s);return(!q||hay.includes(q))&&(mat==="Toutes matières"||s.material===mat)&&(brand==="Toutes les marques"||s.brand===brand)&&(st==="Tous les stocks"||(st==="< 50 %"&&p<50)||(st==="< 25 %"&&p<25)||(st==="< 10 %"&&p<10))})
}
function rowHTML(s){return `<div class=row><div class=dot style="background:${s.color}"></div><div><b>${s.brand} • ${s.material}${s.variant?" "+s.variant:""}</b><div class=sub>${s.colorName||"Sans nom"}</div></div><div class=badge>${Math.round(s.remaining)} g • ${pct(s).toFixed(0)}%</div><div class=actions><button onclick="consume('${s.id}')">Consommer</button><button onclick="editSpool('${s.id}')">Modifier</button></div></div>`}
async function renderSpools(sp){let f=sp.filter(s=>{let q=$("#search").value.toLowerCase(),p=pct(s);return(!q||[s.brand,s.material,s.variant,s.colorName].join(" ").toLowerCase().includes(q))&&($("#matFilter").value==="Toutes matières"||s.material===$("#matFilter").value)&&($("#brandFilter").value==="Toutes les marques"||s.brand===$("#brandFilter").value)&&($("#stockFilter").value==="Tous les stocks"||($("#stockFilter").value==="< 50 %"&&p<50)||($("#stockFilter").value==="< 25 %"&&p<25)||($("#stockFilter").value==="< 10 %"&&p<10))});$("#spoolList").innerHTML=f.map(rowHTML).join("")||`<div class=sub>Aucune bobine.</div>`}
function spoolBlock(s){let p=pct(s), c=s.color||"#888";return `<div class=spool draggable=true data-id="${s.id}" style="--c:${c};--dark:${shade(c,.62)};--pct:${p};color:${contrast(c)}"><b>${s.material}</b><small>${s.colorName||s.brand}</small><em>${p.toFixed(0)}%</em></div>`}
async function renderShelf(sp){
 let f=filtered(sp,"shelf"), by=new Map(f.map(s=>[s.slot??0,s])), max=Math.max(17,...sp.map(s=>s.slot??0)), slots=Math.ceil((max+19)/18)*18, html="";
 for(let panel=0;panel<slots/18;panel++){html+="<div class=rack>";for(let r=0;r<3;r++){html+="<div class=shelf>";for(let c=0;c<6;c++){let slot=panel*18+r*6+c,s=by.get(slot);html+=`<div class=slot data-slot="${slot}">${s?spoolBlock(s):""}</div>`}html+="</div>"}html+="</div>"}$("#racks").innerHTML=html; bindDrag()}
